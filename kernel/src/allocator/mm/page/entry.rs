...
use crate::allocator::mm::{PageFlags, PhysicalAddress};
...
impl<A: Arch> PageEntry<A> {
    ...
    pub fn address(&self) -> Result<PhysicalAddress, PhysicalAddress> {
        let addr = PhysicalAddress(((self.data >> A::ENTRY_ADDRESS_SHIFT) & A::ENTRY_ADDRESS_MASK) << A::PAGE_SHIFT);
        if self.present() {
            Ok(addr)
        } else {
            Err(addr)
    ...
    pub fn flags(&self) -> PageFlags<A> {
        unsafe {
            PageFlags::from_data(self.data & A::ENTRY_FLAGS_MASK)
        }
    }
